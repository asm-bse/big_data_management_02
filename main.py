
import os
import time
import argparse
import gc
from datetime import datetime
from faker import Faker
from pymongo import MongoClient

# ----------------------------------------------------------------------------
# Optimized batch processing functions
# ----------------------------------------------------------------------------

def generate_and_populate_companies_batched(db, fake, total_companies, batch_size=50000):
    comp_coll = db.companies_ref
    comp_coll.drop()
    
    # Create indexes for efficient operations
    comp_coll.create_index([('name', 1)])
    comp_coll.create_index([('founded', 1)])
    comp_coll.create_index([('name', 'text')])
    
    print(f"Generating and inserting {total_companies} companies in batches of {batch_size}...")
    
    all_company_ids = []
    
    for i in range(0, total_companies, batch_size):
        current_batch_size = min(batch_size, total_companies - i)
        
        # Generate company batch
        company_batch = []
        for _ in range(current_batch_size):
            founded_date = fake.date_between(start_date='-50y', end_date='today')
            company_batch.append({
                'name': fake.company(),
                'address': fake.address(),
                'founded': datetime.combine(founded_date, datetime.min.time())
            })
        
        # Bulk insert for performance
        result = comp_coll.insert_many(company_batch, ordered=False)
        all_company_ids.extend(result.inserted_ids)
        
        print(f"Inserted company batch {i//batch_size + 1}/{(total_companies-1)//batch_size + 1}")
        
        del company_batch
        gc.collect()
    
    return all_company_ids

def generate_and_populate_persons_batched(db, fake, total_persons, company_ids, batch_size=100000):
    pers_coll = db.persons_ref
    pers_coll.drop()
    
    # Create indexes for efficient operations
    pers_coll.create_index([('company_id', 1)])  # Main index for joins
    pers_coll.create_index([('birth_date', 1)])  # For date filtering
    pers_coll.create_index([('age', 1)])  # For age filtering
    pers_coll.create_index([('last_name', 1)])  # For lastname search
    pers_coll.create_index([('first_name', 1), ('last_name', 1)])  # Compound index for full name
    
    print(f"Generating and inserting {total_persons} persons in batches of {batch_size}...")
    
    # Precompute random elements for optimization
    num_companies = len(company_ids)
    
    for i in range(0, total_persons, batch_size):
        current_batch_size = min(batch_size, total_persons - i)
        
        # Generate person batch
        person_batch = []
        for _ in range(current_batch_size):
            bd = fake.date_of_birth(minimum_age=18, maximum_age=80)
            age = int((datetime.now().date() - bd).days // 365.25)
            
            person_batch.append({
                'first_name': fake.first_name(),
                'last_name': fake.last_name(),
                'birth_date': datetime.combine(bd, datetime.min.time()),
                'age': age,
                'company_id': company_ids[fake.random_int(0, num_companies-1)]
            })
        
        # Bulk insert for performance
        pers_coll.insert_many(person_batch, ordered=False)
        
        print(f"Inserted person batch {i//batch_size + 1}/{(total_persons-1)//batch_size + 1}")
        
        del person_batch
        gc.collect()

def populate_M2_optimized(db, companies_info=None, persons_info=None, batch_size=5000):
    """Optimized M2 population - reads persons from DB"""
    pers_coll = db.persons_ref
    pers_emb_coll = db.persons_emb
    comp_coll = db.companies_ref
    pers_emb_coll.drop()
    
    # Create indexes for M2 collection
    pers_emb_coll.create_index([('company._id', 1)])  # For joins with company
    pers_emb_coll.create_index([('birth_date', 1)])  # For date filtering
    pers_emb_coll.create_index([('age', 1)])  # For age filtering
    pers_emb_coll.create_index([('first_name', 1), ('last_name', 1)])  # For name search
    pers_emb_coll.create_index([('company.name', 1)])  # For company name search
    
    print(f"Creating M2: persons with embedded company data (batch size: {batch_size})...")
    
    # Count total persons for progress tracking
    total_persons = pers_coll.count_documents({})
    processed = 0
    
    # Process persons in batches using cursor
    cursor = pers_coll.find().batch_size(batch_size)
    
    batch_persons = []
    for person in cursor:
        batch_persons.append(person)
        
        if len(batch_persons) >= batch_size:
            # Process batch
            persons_emb = []
            
            # Get unique company IDs for this batch
            company_ids = list(set(p['company_id'] for p in batch_persons))
            
            # Fetch company data for this batch only
            companies_cursor = comp_coll.find({'_id': {'$in': company_ids}})
            company_dict = {c['_id']: c for c in companies_cursor}
            
            for p in batch_persons:
                comp = company_dict[p['company_id']]
                persons_emb.append({
                    'first_name': p['first_name'],
                    'last_name': p['last_name'],
                    'birth_date': p['birth_date'],
                    'age': p['age'],
                    'company': {'_id': comp['_id'], 'name': comp['name'], 'address': comp['address']}
                })
            
            pers_emb_coll.insert_many(persons_emb, ordered=False)
            processed += len(batch_persons)
            print(f"Inserted M2 batch: {processed}/{total_persons} persons processed")
            
            # Clear batch
            batch_persons = []
    
    # Process remaining persons
    if batch_persons:
        persons_emb = []
        
        # Get unique company IDs for this batch
        company_ids = list(set(p['company_id'] for p in batch_persons))
        
        # Fetch company data for this batch only
        companies_cursor = comp_coll.find({'_id': {'$in': company_ids}})
        company_dict = {c['_id']: c for c in companies_cursor}
        
        for p in batch_persons:
            comp = company_dict[p['company_id']]
            persons_emb.append({
                'first_name': p['first_name'],
                'last_name': p['last_name'],
                'birth_date': p['birth_date'],
                'age': p['age'],
                'company': {'_id': comp['_id'], 'name': comp['name'], 'address': comp['address']}
            })
        
        pers_emb_coll.insert_many(persons_emb, ordered=False)
        processed += len(batch_persons)
        print(f"Inserted M2 final batch: {processed}/{total_persons} persons processed")
    
    print(f"M2 populated: {total_persons} persons with embedded company data")

def populate_M3_optimized(db, companies_info, persons_info=None, batch_size=500):
    """Optimized M3 population - reads persons from DB using aggregation"""
    comp_coll = db.companies_emb
    pers_coll = db.persons_ref
    comp_coll.drop()
    
    # Create indexes for M3 collection
    comp_coll.create_index([('name', 1)])  # For company name search
    comp_coll.create_index([('founded', 1)])  # For founded date filtering
    comp_coll.create_index([('employees.last_name', 1)])  # For employee lastname search
    comp_coll.create_index([('employees.birth_date', 1)])  # For employee birth date filtering
    comp_coll.create_index([('name', 'text')])  # For text search
    
    print(f"Creating M3: companies with embedded employees using aggregation pipeline...")
    
    # Use aggregation to group employees by company efficiently
    pipeline = [
        {
            '$group': {
                '_id': '$company_id',
                'employees': {
                    '$push': {
                        'first_name': '$first_name',
                        'last_name': '$last_name',
                        'birth_date': '$birth_date',
                        'age': '$age'
                    }
                }
            }
        }
    ]
    
    print("Aggregating employees by company...")
    company_employees = {}
    for result in pers_coll.aggregate(pipeline, allowDiskUse=True):
        company_employees[result['_id']] = result['employees']
    
    print(f"Creating M3 documents in batches of {batch_size}...")
    
    # Get companies from DB instead of from memory
    companies_cursor = db.companies_ref.find()
    companies_batch = []
    batch_count = 0
    
    for comp in companies_cursor:
        companies_batch.append(comp)
        
        if len(companies_batch) >= batch_size:
            comp_docs = []
            
            for c in companies_batch:
                employees = company_employees.get(c['_id'], [])
                comp_docs.append({
                    'name': c['name'],
                    'address': c['address'],
                    'founded': c['founded'],
                    'employees': employees
                })
            
            comp_coll.insert_many(comp_docs, ordered=False)
            batch_count += 1
            print(f"Inserted M3 batch {batch_count}")
            
            # Clear batch
            companies_batch = []
    
    # Process remaining companies
    if companies_batch:
        comp_docs = []
        
        for c in companies_batch:
            employees = company_employees.get(c['_id'], [])
            comp_docs.append({
                'name': c['name'],
                'address': c['address'],
                'founded': c['founded'],
                'employees': employees
            })
        
        comp_coll.insert_many(comp_docs, ordered=False)
        batch_count += 1
        print(f"Inserted M3 final batch {batch_count}")
    
    print(f"M3 populated: {batch_count * batch_size} companies with embedded employees")

# ----------------------------------------------------------------------------
# Data generation
# ----------------------------------------------------------------------------

def generate_companies(fake, count):
    companies = []
    for _ in range(count):
        fd = fake.date_between(start_date='-50y', end_date='today')
        founded = datetime.combine(fd, datetime.min.time())
        companies.append({
            'name': fake.company(),
            'address': fake.address(),
            'founded': founded
        })
    return companies


def generate_persons(fake, count, company_ids):
    persons = []
    for _ in range(count):
        bd = fake.date_of_birth(minimum_age=18, maximum_age=80)
        birth = datetime.combine(bd, datetime.min.time())
        age = int((datetime.now().date() - bd).days // 365)
        cid = fake.random_element(company_ids)
        persons.append({
            'first_name': fake.first_name(),
            'last_name': fake.last_name(),
            'birth_date': birth,
            'age': age,
            'company_id': cid
        })
    return persons

# ----------------------------------------------------------------------------
# Populate pipelines
# ----------------------------------------------------------------------------

def populate_M1(db, companies, persons, batch_size=10000):
    comp_coll = db.companies_ref
    pers_coll = db.persons_ref
    comp_coll.drop()
    pers_coll.drop()
    
    # Insert companies in batches
    print(f"Inserting {len(companies)} companies in batches of {batch_size}...")
    comp_ids = []
    for i in range(0, len(companies), batch_size):
        batch = companies[i:i+batch_size]
        batch_ids = comp_coll.insert_many(batch).inserted_ids
        comp_ids.extend(batch_ids)
        print(f"Inserted companies batch {i//batch_size + 1}/{(len(companies)-1)//batch_size + 1}")
    
    # Update companies with their IDs
    for comp, _id in zip(companies, comp_ids): 
        comp['_id'] = _id
    
    # Insert persons in batches
    print(f"Inserting {len(persons)} persons in batches of {batch_size}...")
    for i in range(0, len(persons), batch_size):
        batch = persons[i:i+batch_size]
        pers_coll.insert_many(batch)
        print(f"Inserted persons batch {i//batch_size + 1}/{(len(persons)-1)//batch_size + 1}")
    
    dist = list(pers_coll.aggregate([
        {'$group': {'_id': '$company_id', 'count': {'$sum': 1}}},
        {'$sort': {'count': -1}}
    ]))
    print("\nM1 populated. Top 5 companies by person count:")
    for d in dist[:5]: print(f"Company {d['_id']}: {d['count']}")
    print(f"Avg persons per company: {len(persons)/len(companies):.2f}\n")


def populate_M2(db, companies, persons, batch_size=10000):
    pers_coll = db.persons_emb
    pers_coll.drop()
    
    print(f"Creating M2: persons with embedded company data...")
    print(f"Processing {len(persons)} persons in batches of {batch_size}...")
    
    for i in range(0, len(persons), batch_size):
        batch_persons = persons[i:i+batch_size]
        persons_emb = []
        
        for p in batch_persons:
            comp = next(c for c in companies if c['_id'] == p['company_id'])
            persons_emb.append({
                'first_name': p['first_name'],
                'last_name': p['last_name'],
                'birth_date': p['birth_date'],
                'age': p['age'],
                'company': {'_id': comp['_id'], 'name': comp['name'], 'address': comp['address']}
            })
        
        pers_coll.insert_many(persons_emb)
        print(f"Inserted M2 batch {i//batch_size + 1}/{(len(persons)-1)//batch_size + 1}")
    
    print(f"M2 populated: persons with embedded company. Count: {len(persons)}")


def populate_M3(db, companies, persons, batch_size=1000):
    comp_coll = db.companies_emb
    comp_coll.drop()
    
    print(f"Creating M3: companies with embedded employees...")
    print(f"Processing {len(companies)} companies in batches of {batch_size}...")
    
    for i in range(0, len(companies), batch_size):
        batch_companies = companies[i:i+batch_size]
        comp_docs = []
        
        for comp in batch_companies:
            emps = [p for p in persons if p['company_id'] == comp['_id']]
            comp_docs.append({
                'name': comp['name'],
                'address': comp['address'],
                'founded': comp['founded'],
                'employees': [{'first_name': p['first_name'], 'last_name': p['last_name'], 'birth_date': p['birth_date'], 'age': p['age']} for p in emps]
            })
        
        comp_coll.insert_many(comp_docs)
        print(f"Inserted M3 batch {i//batch_size + 1}/{(len(companies)-1)//batch_size + 1}")
    
    print(f"M3 populated: companies with embedded persons. Count: {len(companies)}")

# ----------------------------------------------------------------------------
# Query pipelines
# ----------------------------------------------------------------------------

def run_queries_M1(db):
    comp_coll = db.companies_ref
    pers_coll = db.persons_ref
    print("\n=== M1 Queries ===")
    # Q1: lookup persons with company names
    start = time.time()
    q1 = list(pers_coll.aggregate([
        {'$lookup': {'from': 'companies_ref', 'localField': 'company_id', 'foreignField': '_id', 'as': 'company'}},
        {'$unwind': '$company'},
        {'$project': {'full_name': {'$concat': ['$first_name',' ', '$last_name']}, 'company_name': '$company.name', '_id': 0}}
    ]))
    t1 = time.time() - start
    print(f"Q1: {len(q1)} docs, time={t1:.4f}s")
    print("Q1 sample:", q1[:3])
    # Q2: count employees per company (including companies with 0 employees)
    start = time.time()
    q2 = list(comp_coll.aggregate([
        {
            '$lookup': {
                'from': 'persons_ref',
                'localField': '_id',
                'foreignField': 'company_id',
                'as': 'employees'
            }
        },
        {
            '$project': {
                'name': 1,
                'num_employees': {'$size': '$employees'},
                '_id': 0
            }
        },
        {'$sort': {'num_employees': -1}}
    ]))
    t2 = time.time() - start
    print(f"Q2: {len(q2)} companies, time={t2:.4f}s")
    print("Q2 sample:", q2[:3])
    # Q3: update ages for those born before 1988
    start = time.time()
    result3 = pers_coll.update_many({'birth_date': {'$lt': datetime(1988,1,1)}}, {'$set': {'age':30}})
    t3 = time.time() - start
    print(f"Q3: modified {result3.modified_count} docs, time={t3:.4f}s")
    # Q4: append ' Company' to company names (only if not already present)
    start = time.time()
    result4 = comp_coll.update_many(
        {'name': {'$not': {'$regex': ' Company$'}}},
        [{'$set': {'name': {'$concat': ['$name',' Company']}}}]
    )
    t4 = time.time() - start
    print(f"Q4: modified {result4.modified_count} docs, time={t4:.4f}s")


def run_queries_M2(db):
    pers_coll = db.persons_emb
    print("\n=== M2 Queries ===")
    # Q1: full names with company names
    start = time.time()
    q1 = list(pers_coll.aggregate([
        {'$project': {'_id': 0, 'full_name': {'$concat': ['$first_name',' ', '$last_name']}, 'company.name':1}}
    ]))
    t1 = time.time() - start
    print(f"Q1: {len(q1)} docs, time={t1:.4f}s")
    print("Q1 sample:", q1[:3])
    # Q2: count of employees per company (including companies with 0 employees)
    start = time.time()
    # Start from companies_ref to include ALL companies (even those with 0 employees)
    comp_coll = db.companies_ref
    q2 = list(comp_coll.aggregate([
        {
            '$lookup': {
                'from': 'persons_emb',
                'localField': '_id',
                'foreignField': 'company._id',
                'as': 'employees'
            }
        },
        {
            '$project': {
                '_id': 0,
                'name': 1,
                'num_employees': {'$size': '$employees'}
            }
        },
        {'$sort': {'num_employees': -1}}
    ]))
    t2 = time.time() - start
    print(f"Q2: {len(q2)} companies, time={t2:.4f}s")
    print("Q2 sample:", q2[:3])
    # Q3: update ages for those born before 1988
    start = time.time()
    result3 = pers_coll.update_many({'birth_date': {'$lt': datetime(1988,1,1)}}, {'$set': {'age':30}})
    t3 = time.time() - start
    print(f"Q3: modified {result3.modified_count} docs, time={t3:.4f}s")
    # Q4: append ' Company' to company names (only if not already present)
    start = time.time()
    result4 = pers_coll.update_many(
        {'company.name': {'$not': {'$regex': ' Company$'}}},
        [{'$set': {'company.name': {'$concat': ['$company.name',' Company']}}}]
    )
    t4 = time.time() - start
    print(f"Q4: modified {result4.modified_count} docs, time={t4:.4f}s")


def run_queries_M3(db):
    comp_coll = db.companies_emb
    print("\n=== M3 Queries ===")
    # Q1: unwind employees to get full names
    start = time.time()
    q1 = list(comp_coll.aggregate([
        {'$unwind': '$employees'},
        {'$project': {'full_name': {'$concat': ['$employees.first_name',' ', '$employees.last_name']}, 'company_name': '$name', '_id':0}}
    ]))
    t1 = time.time() - start
    print(f"Q1: {len(q1)} docs, time={t1:.4f}s")
    print("Q1 sample:", q1[:3])
    # Q2: count employees per company (including companies with 0 employees)
    start = time.time()
    q2 = list(comp_coll.aggregate([
        {'$project': {'name':1,'num_employees': {'$size':'$employees'}, '_id':0}},
        {'$sort': {'num_employees': -1}}
    ]))
    t2 = time.time() - start
    print(f"Q2: {len(q2)} companies, time={t2:.4f}s")
    print("Q2 sample:", q2[:3])
    # Q3: update embedded ages for those born before 1988
    start = time.time()
    result3 = comp_coll.update_many(
        {'employees.birth_date': {'$lt': datetime(1988,1,1)}},
        {'$set': {'employees.$[e].age':30}}, array_filters=[{'e.birth_date': {'$lt': datetime(1988,1,1)}}]
    )
    t3 = time.time() - start
    print(f"Q3: modified {result3.modified_count} docs, time={t3:.4f}s")
    # Q4: append ' Company' to company names (only if not already present)
    start = time.time()
    result4 = comp_coll.update_many(
        {'name': {'$not': {'$regex': ' Company$'}}},  # Только если нет " Company" в конце
        [{'$set': {'name': {'$concat': ['$name',' Company']}}}]
    )
    t4 = time.time() - start
    print(f"Q4: modified {result4.modified_count} docs, time={t4:.4f}s")

# ----------------------------------------------------------------------------
# Main
# ----------------------------------------------------------------------------
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Lab1 MongoDB: populate or run queries')
    sub = parser.add_subparsers(dest='action', required=True)
    pop = sub.add_parser('populate', help='Create and populate collections')
    pop.add_argument('--uri', type=str, default=os.getenv('MONGODB_URI','mongodb://localhost:27017'))
    pop.add_argument('--db', type=str, default='lab1_db')
    pop.add_argument('--n_companies', type=int, default=5)
    pop.add_argument('--n_persons', type=int, default=50000)
    qry = sub.add_parser('queries', help='Run queries on existing data')
    qry.add_argument('--uri', type=str, default=os.getenv('MONGODB_URI','mongodb://localhost:27017'))
    qry.add_argument('--db', type=str, default='lab1_db')
    args = parser.parse_args()

    client = MongoClient(args.uri, 
                        maxPoolSize=50,
                        waitQueueTimeoutMS=2500,
                        retryWrites=True)
    db = client[args.db]

    if args.action == 'populate':
        fake = Faker()
        
        # Use optimized batch processing for large datasets
        if args.n_companies > 10000 or args.n_persons > 100000:
            print("Using optimized batch processing for large dataset...")
            
            # Generate and populate companies directly to DB
            company_ids = generate_and_populate_companies_batched(
                db, fake, args.n_companies, batch_size=min(50000, args.n_companies//100)
            )
            
            # Generate and populate persons directly to DB with smaller batches
            generate_and_populate_persons_batched(
                db, fake, args.n_persons, company_ids, batch_size=min(50000, args.n_persons//1000)
            )
            
            print("M1 already populated during batch processing")
            
            # Populate M2 and M3 with optimized functions
            populate_M2_optimized(db, batch_size=25000)
            populate_M3_optimized(db, None, batch_size=2500)
            
        else:
            # Original approach for smaller datasets
            print("Using standard processing for small dataset...")
            companies = []
            batch_size_companies = min(10000, args.n_companies)
            
            # Create temp collection to get company IDs
            temp_coll = db['__temp_comp']
            temp_coll.drop()
            
            for i in range(0, args.n_companies, batch_size_companies):
                batch_count = min(batch_size_companies, args.n_companies - i)
                company_batch = generate_companies(fake, batch_count)
                batch_ids = temp_coll.insert_many(company_batch).inserted_ids
                
                # Add IDs to companies and store them
                for comp, _id in zip(company_batch, batch_ids):
                    comp['_id'] = _id
                companies.extend(company_batch)
                
                print(f"Generated company batch {i//batch_size_companies + 1}/{(args.n_companies-1)//batch_size_companies + 1}")
            
            temp_coll.drop()
            comp_ids = [c['_id'] for c in companies]
            
            # Generate persons in batches
            print(f"Generating {args.n_persons} persons...")
            persons = []
            batch_size_persons = min(50000, args.n_persons)
            
            for i in range(0, args.n_persons, batch_size_persons):
                batch_count = min(batch_size_persons, args.n_persons - i)
                person_batch = generate_persons(fake, batch_count, comp_ids)
                persons.extend(person_batch)
                print(f"Generated person batch {i//batch_size_persons + 1}/{(args.n_persons-1)//batch_size_persons + 1}")
            
            print(f"Generated {len(companies)} companies and {len(persons)} persons.")
            
            # Populate models with smaller batch sizes for large datasets
            batch_size_insert = 50000 if args.n_persons > 100000 else 100000
            populate_M1(db, companies, persons, batch_size_insert)
            populate_M2(db, companies, persons, batch_size_insert)
            populate_M3(db, companies, persons, min(5000, len(companies)//10))  # Smaller batches for M3
    else:
        run_queries_M1(db)
        run_queries_M2(db)
        run_queries_M3(db)
